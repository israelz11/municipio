			...
			this.getTransactionTemplate().execute(new TransactionCallbackWithoutResult(){
	            ...
	            protected void   doInTransactionWithoutResult(TransactionStatus status) {
	            	...
	            	for(Long cve_factura: cve_facturas){
	            		...
	            		int contv =getJdbcTemplate().queryForInt("SELECT COUNT(*) FROM SAM_FACTURA_MOV_RETENC WHERE CVE_FACTURA =?", new Object[]{cve_factura});;
	            		...
	            		for(Map row: detallesVales){
	            			...
	            			getJdbcTemplate().update("INSERT INTO COMP_VALES (CVE_VALE, CONS_VALE, CVE_OP, TIPO, ID_PROYECTO, CLV_PARTID, IMPORTE, IMP_ANTERIOR, IMP_PENDIENTE, FECHA) " +
	            					"VALUES (?,?,?,?,?,?,?,?,?,?)"
	            					, new Object[]{row.get("CVE_VALE"), contv, cve_op, "OP", row.get("ID_PROYECTO"), row.get("CLV_PARTID"), row.get("IMPORTE"), 0,0, new Date()});
	            		}
	            	}
