            		...
            		if (tipo==2){//Requisiciones
    	                ...
    	                for(Map row: lst_doc)
        	    		{
    	                	...
    	                	if(row.get("CVE_CONTRATO_REQ")!=null)
    	                		getJdbcTemplate().update("INSERT INTO SAM_COMP_CONTRATO(CVE_CONTRATO, TIPO_MOV, TIPO_DOC, CVE_DOC, PERIODO, IMPORTE) VALUES(?,?,?,?,?,?) ", new Object[]{row.get("CVE_CONTRATO_REQ"), "LIBERACION", tipo_doc, row.get("CVE_REQ"), row.get("PERIODO"), row.get("TOTAL")});
        	    		}
            		}*/
...
public boolean cambiarFechaOrdenPago(Long cve_op, String fechaNueva, int ejercicio, int cve_pers){
	...
	public String GenerarOrdenPagoxDevengo(final Long cve_op, final List<Long> cve_facturas,final int ejercicio, final int cve_pers){	
	//public String GenerarOrdenPagoxDevengo(final Long cve_op, final Long[] cve_facturas,final int ejercicio, final int cve_pers,String reembolsoFondo,String  concurso,String  nota,int  status,Integer  cveRequisicion, double importeIva, String cveUnidad , Integer periodo, int tipoGasto, Integer idGrupo, Long cve_contrato){
			try { 
				final String result = ""; 
				this.getTransactionTemplate().execute(new TransactionCallbackWithoutResult(){
		            @Override
