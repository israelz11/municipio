...
@Controller
@RequestMapping("/correspondencia/documental/_subirArchivo.action")
public class ControladorTargetSubirArchivoCorrespondencia extends ControladorBaseCorrespondencia {
	...
	public ControladorTargetSubirArchivoCorrespondencia(){
	...
	public String requestPostControlador(Map modelo, HttpServletRequest request, HttpServletResponse response, @RequestParam("archivo") MultipartFile file) throws IOException  {
		...
		Map m = new HashMap();
		String json = "";
		Long idCorrespondencia = ((request.getParameter("ID_CORRESPONDENCIA")==null||request.getParameter("ID_CORRESPONDENCIA")=="") ? 0L : Long.parseLong(request.getParameter("ID_CORRESPONDENCIA").toString()));
		m.put("mensaje", false);
		if(idCorrespondencia!=0){
			if (file !=null && file.getSize()> 0 ){
			  	...
			  	String path = request.getSession().getServletContext().getRealPath("")+"/correspondencia/archivos/correspondencia/";	  	
			  	...
			  	Long idArchivo = gatewayCorrespondencia.guardarArchivo(idCorrespondencia, 0L, nombreArchivo, "/archivos/correspondencia/", new Date(), ext, size);
		...
		return "correspondencia/documental/_subirArchivo.jsp";
	}
