...
public class ControladorMuestraPedidoFacturas extends BaseGateway {
	
	
	@Autowired
	GatewayFacturas gatewayFacturas;
	
	public ControladorMuestraPedidoFacturas() {
	 ...
	 public List<Map> getListaPedidos(int idDependencia){
	    return this.getJdbcTemplate().queryForList("SELECT CVE_PED, "+
												   ...
												   "INNER JOIN SAM_REQUISIC ON (SAM_REQUISIC.CVE_REQ = SAM_PEDIDOS_EX.CVE_REQ)  "+
														...
														"WHERE SAM_PEDIDOS_EX.STATUS IN(1, 4) AND (SAM_PEDIDOS_EX.TOTAL -ISNULL((SELECT SUM(F.TOTAL) FROM SAM_FACTURAS F WHERE F.STATUS IN (1,3) AND F.CVE_PED=SAM_PEDIDOS_EX.CVE_PED),0.00))>0 AND CAT_DEPENDENCIAS.ID = ? ORDER BY CVE_PED ASC", new Object[]{idDependencia});
	}
